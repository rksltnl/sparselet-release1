function [ds, bs, conv_time, trees] = gdetect(pyra, model, thresh, max_num)
...
%th = tic();
[model, conv_time] = gdetect_dp(pyra, model);
%[model, conv_time] = gdetect_dp(pyra, model);
%[model, conv_time] = gdetect_dp_parallel_DT(pyra, model);
%fprintf('gdetect_dp: %.3f\n', toc(th));
%th = tic();
[ds, bs, trees] = gdetect_parse(model, pyra, thresh, max_num);
%fprintf('gdetect_parse: %.3f\n', toc(th));
