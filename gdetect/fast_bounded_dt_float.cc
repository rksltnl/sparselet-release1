...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  float ax = (float)mxGetScalar(prhs[IN_AX]);
  float bx = (float)mxGetScalar(prhs[IN_BX]);
  float ay = (float)mxGetScalar(prhs[IN_AY]);
  float by = (float)mxGetScalar(prhs[IN_BY]);
  float range = (float)mxGetScalar(prhs[IN_RANGE]);
